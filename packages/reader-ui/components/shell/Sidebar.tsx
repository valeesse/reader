...
export function Sidebar({ currentView, onChangeView }: SidebarProps) {
  ...
  return (
    <aside className="app-sidebar fixed inset-x-2 bottom-2 z-40 flex h-[calc(3.75rem+env(safe-area-inset-bottom))] shrink-0 flex-row overflow-hidden rounded-2xl glass-surface px-1.5 pb-[env(safe-area-inset-bottom)] sm:static sm:h-full sm:flex-col sm:rounded-2xl sm:px-0 sm:pb-2 sm:pt-5">
      <div className="hidden px-3 pb-7 sm:block min-[1280px]:px-5">
        <h1 className="flex h-9 items-center justify-center gap-2.5 text-lg font-semibold text-[#1C1C1E] dark:text-[#f0f0eb] min-[1280px]:justify-start">
          ...
          <span className="hidden truncate min-[1280px]:block">Zenith</span>
        </h1>
      </div>
      ...
      <nav aria-label="主导航" className="flex h-full flex-1 items-stretch gap-1 sm:block sm:h-auto sm:space-y-1.5 sm:overflow-y-auto sm:px-2.5">
        <div className="hidden px-3 pb-2 text-[11px] font-semibold text-black/50 dark:text-white/50 min-[1280px]:block">
          我的阅读
        </div>
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                'group flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-1 text-[11px] font-medium transition-colors sm:h-10 sm:w-full sm:flex-row sm:gap-3 sm:px-3 sm:text-sm min-[1280px]:justify-start',
                active
            ...
            >
              ...
              <span className="hidden truncate min-[1280px]:block">{item.label}</span>
              {typeof item.count === 'number' && (
                <span className={cn(
                  'ml-auto hidden rounded-full px-2 py-0.5 text-xs tabular-nums min-[1280px]:block',
      ...
      <div className="mt-auto hidden h-14 items-center justify-center border-t border-black/[0.06] px-2.5 dark:border-white/[0.06] sm:flex min-[1280px]:justify-start">
        <button
          onClick={() => onChangeView('settings')}
          className={cn(
            'flex h-10 w-10 items-center justify-center rounded-xl transition-colors min-[1280px]:w-full min-[1280px]:justify-start min-[1280px]:gap-3 min-[1280px]:px-3',
            currentView === 'settings'
        ...
        >
          ...
          <span className="hidden text-sm font-medium min-[1280px]:block">偏好设置</span>
        </button>
      </div>
    </aside>
  );
